...
### 2.6. Обработка ошибок в интеграциях микросервисов

**Цель:** Обеспечить отказоустойчивость системы при сбоях во взаимодействии микросервисов, предотвратить каскадные отказы и гарантировать обработку критичных бизнес-операций.

#### 2.6.1. Синхронные HTTP/REST интеграции (Circuit Breaker Pattern)

**Контекст:** Интеграции, требующие немедленного ответа (оплата, проверка доступности, расчет доставки).

**Сценарии ошибок:**
- **Timeout** (100ms) - сервис не отвечает в установленное время
- **5xx ошибки** - внутренние проблемы зависимого сервиса
- **4xx ошибки** - клиентские ошибки (некорректные данные)

      -Состояния:
      -CLOSED: Нормальная работа, запросы проходят  
      -OPEN: Блокировка запросов при 5 ошибках за 60 секунд  
      -HALF-OPEN: Тестовые запросы после 30 секунд блокировки


2. **Retry Pattern с экспоненциальной задержкой:**
- 2 повторные попытки с задержкой: 1s > 3s
- Применяется только для временных ошибок (5xx, timeout)

3. **Fallback Mechanism:**
- Использование кешированных данных (исторические тарифы, старые цены)
- Graceful degradation - продолжение работы с ограниченной функциональностью
- Альтернативные маршруты выполнения операции

**Применение в системе:**
- **Payment Service > External Payment Gateways** (платежные шлюзы)
- **Order Service > Delivery Service** (расчет доставки)
- **Health Metrics Service > External Device APIs** (медицинские устройства)

#### 2.6.2. Асинхронные интеграции с очередями (Dead Letter Queue Pattern)

**Контекст:** Критичные бизнес-операции, требующие гарантированной обработки (резервирование товаров, уведомления, аналитика).

**Сценарии ошибок:**
- **Временная недоступность** сервиса или БД
- **Бизнес-логика:** недостаток товара, конфликтующие операции
- **Невалидные данные:** некорректный формат сообщения
- **Таймаут обработки:** операция занимает слишком много времени

**Стратегии обработки:**
1. **Retry с Exponential Backoff:**

**Стратегии обработки:**
1. **Circuit Breaker Pattern (Реализация: Resilience4j):**

- График повторных попыток:
1-я ошибка: повтор через 30 секунд
2-я ошибка: повтор через 2 минуты
3-я ошибка: повтор через 5 минут
Максимум: 3 попытки
...
- Сохранение для аудита и пост-мортем анализа
- Мониторинг размера DLQ и автоматические алерты

3. **Compensating Actions :**
- При неудачном резервировании товара → отмена заказа
- При таймауте оплаты → автоматическая отмена резервирования
- Последовательная компенсация всех выполненных шагов

4. **Мониторинг и алертинг:**
- Уведомление при накоплении >100 сообщений в DLQ
- Мониторинг времени обработки сообщений (P95 < 5s)
- Отслеживание успешности операций (SLO: 99.9%)

**Применение в системе:**
- **Order Service > Inventory Service** (резервирование товаров)
- **Event Service > Notification Service** (массовые уведомления)
- **Analytics Service > Data Warehouse** (синхронизация аналитических данных)

#### 2.6.3. Критерии выбора стратегий

**Для синхронных HTTP-интеграций выбраны:** Circuit Breaker + Retry + Fallback, потому что:
- Быстрое обнаружение и изоляция проблемных сервисов
- Автоматическое восстановление при временных сбоях
- Минимальное влияние на пользовательский опыт

**Для асинхронных интеграций выбраны:** Retry + DLQ + Compensating Actions, потому что:
- Гарантируют обработку команд даже при временных сбоях
- Изолируют проблемные сообщения для ручного вмешательства
- Обеспечивают согласованность данных через компенсирующие транзакции
- Подходят для критичных бизнес-процессов, где надежность важнее скорости

#### 2.6.4. Мониторинг и наблюдаемость

**Метрики для мониторинга:**
- Состояние Circuit Breaker (OPEN/CLOSED/HALF-OPEN по каждому сервису)
- Количество сообщений в DLQ по топикам
- Время обработки сообщений (среднее, P95, P99)
- Успешность повторных попыток (retry success rate)
- Задержка между попытками (retry latency)

**Дашборды:**
- **Интеграционное здоровье:** статус всех Circuit Breaker
- **Очереди обработки:** размеры DLQ, время обработки
- **Компенсирующие транзакции:** количество и причины откатов
...
Система проектируется с учетом возможных сбоев и продолжает работать при частичных отказах. Для внешних зависимостей применяются паттерны Circuit Breaker, Retry, Fallback, а для внутренних взаимодействий — Dead Letter Queue и компенсирующие транзакции (см. раздел 2.6).
