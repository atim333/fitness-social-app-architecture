...
**Конфигурация Dead Letter Queue (DLQ) для обработки ошибок:**
- **Топики DLQ:** `dlq.event-registrations`, `dlq.workout-data-stream`, `dlq.social-interactions`
- **Стратегия повторных попыток:** 3 попытки с exponential backoff (30s → 2m → 5m)
- **Мониторинг DLQ:** Алертинг при накоплении > 100 сообщений в DLQ
- **Автоматическая обработка:** Фоновые процессы для анализа и повторной обработки сообщений из DLQ
...
## 4. Event Sourcing и CQRS инфраструктура

### 4.1 Apache Kafka для Event Streaming

**Архитектура Kafka для Event Sourcing:**

**Кластерная конфигурация:**
- **Брокеры:** Минимум 3 брокера в каждом регионе для отказоустойчивости
- **Репликация:** Replication factor 3 для критичных топиков событий
- **Партиционирование:** По aggregateId для сохранения порядка событий агрегата
- **Retention policy:** 90 дней для доменных событий, 7 дней для технических логов

**Разделение топиков по типам событий:**
- **Бизнес-события:** `business.events` - доменные события (OrderCreated, PaymentCompleted)
- **Технические события:** `technical.events` - метрики, аудит, системные события
- **Логи приложений:** `application.logs` - логи для отладки и мониторинга

**Конфигурация топиков Event Sourcing:**
- **Топик для Order событий:** `orders.events` с 12 партициями
- **Топик для Payment событий:** `payments.events` с 8 партициями
- **Топик для Inventory событий:** `inventory.events` с 10 партициями
- **Топик для аналитики:** `analytics.events` с 20 партициями

**Гарантии доставки событий:**
- **Exactly-once семантика:** Для критичных финансовых операций
- **At-least-once семантика:** Для большинства бизнес-событий
- **Идемпотентные производители:** Предотвращение дублирования событий
- **Транзакционность:** Атомарная запись событий в несколько топиков

### 4.2 Хранилища для CQRS Read Models
...
- **Хранение данных:** 30 дней горячих данных, 1 год архивных данных

**Read Models в ClickHouse:**
- **OrderAnalytics:** Агрегированные данные по заказам для отчетности
- **CustomerFunnel:** Воронки продаж и поведения пользователей
- **InventoryStatus:** Текущее состояние остатков по всем складам
- **PaymentAnalytics:** Статистика по платежам и конверсии

**MongoDB для оперативных Read Models:**

**Конфигурация MongoDB для CQRS:**
- **Шардированный кластер:** 3 шарда с 3 репликами каждый
- **Индексы:** Составные индексы для быстрых запросов по пользователям
- **TTL индексы:** Автоматическое удаление устаревших данных

**Read Models в MongoDB:**
- **OrderSummary:** Денормализованное представление заказов для UI
- **CustomerProfile:** Агрегированные данные по клиентам
- **ProductCatalog:** Кэшированный каталог товаров с актуальными ценами

**Redis для горячих Read Models:**

**Использование Redis в CQRS:**
- **Кэширование:** Горячие данные для снижения нагрузки на БД
- **Сессионные данные:** Текущее состояние пользовательских сессий
- **Рейтинги и лидерборды:** Real-time обновления для геймификации
- **Геоданные:** Кэш для поиска тренировок и событий поблизости

### 4.3 Akka Cluster для Actor Model

**Архитектура Akka Cluster:**

**Конфигурация кластера:**
- **Seed nodes:** 3 seed узла в каждой зоне доступности
- **Членство узлов:** Автоматическое обнаружение через gossip протокол
- **Шардирование:** Akka Cluster Sharding для распределения агрегатов
- **Распределение:** Consistent hashing по aggregateId

**Реализация агрегатов как Akka Actors:**

**OrderActor (Event Sourced):**
- **Состояние:** Восстанавливается из событий при старте
- **Snapshotting:** Периодические снепшоты каждые 100 событий
- **Локализация:** Шардирование по orderId
- **Супервизия:** Автоматический перезапуск при сбоях

**PaymentActor (Event Sourced):**
- **Транзакционность:** Гарантированная обработка платежных операций
- **Компенсационные транзакции:** Поддержка саг для откатов
- **Аудит:** Полная история изменений для compliance

**ProductStockActor (Event Sourced):**
- **Конкурентность:** Оптимистичные блокировки через версионирование
- **Резервирование:** Управление временными резервами с TTL
- **Согласованность:** Eventual consistency между репликами

**Управление состоянием акторов:**
- **Event journal:** Kafka как хранилище событий для акторов
- **Snapshot store:** S3/GCS для периодических снепшотов
- **In-memory состояние:** Горячее состояние в памяти для производительности
- **Восстановление:** Реиграция событий при перезапуске актора

### 4.4 Мониторинг Event Streams

**Метрики Kafka для мониторинга:**

**Потребление событий:**
- **Consumer lag:** Отставание консьюмеров по партициям
- **Throughput:** Количество событий в секунду на топик
- **Latency:** Задержка между публикацией и потреблением события
- **Error rate:** Процент ошибок при обработке событий

**Мониторинг состояния акторов:**
- **Количество активных акторов:** По типам агрегатов
- **Сообщения в секунду:** Интенсивность обработки команд
- **Время восстановления:** Длительность реиграции событий при старте
- **Размер event journal:** Количество событий на агрегат

**Алертинг для Event Sourcing:**
- **High consumer lag:** Отставание более 10,000 событий
- **Event processing errors:** Более 5% ошибок при обработке
- **Actor recovery failures:** Сбои при восстановлении состояния
- **Event journal growth:** Аномальный рост размера журналов

### 4.5 Резервное копирование и восстановление Event Stores

**Стратегии для Kafka:**
- **MirrorMaker 2:** Репликация топиков между регионами
- **Point-in-time recovery:** Восстановление на определенный момент времени
- **Тематические бэкапы:** Выборочное копирование критичных топиков
- **Автоматическое восстановление:** Скрипты для быстрого восстановления

**Снепшоты для Akka Actors:**
- **Периодичность:** Каждые 100 событий или 24 часа
- **Хранение:** Объектные хранилища (S3/GCS/Azure Blob)
- **Шифрование:** Customer-managed keys для чувствительных данных
- **Валидация:** Проверка целостности снепшотов

**Disaster Recovery для Event Sourcing:**
- **Multi-region репликация:** Активные реплики в standby регионах
- **Warm standby:** Подготовленные инстансы для быстрого переключения
- **Тестирование восстановления:** Ежеквартальные DR drills
- **Документация:** Подробные runbooks для восстановления
----
## 5. Kubernetes и контейнеризация

### 5.1 Архитектура Kubernetes кластеров
...
### 5.2 Стратегия развертывания контейнеров
...
- 
**Circuit breaker конфигурация для критических сервисов:**
- **Порог срабатывания:** 5 ошибок за 60 секунд
- **Состояния:** CLOSED → OPEN (30 секунд) → HALF-OPEN
- **Fallback стратегии:** Использование кешированных данных или упрощенной логики
- **Мониторинг:** Prometheus метрики для отслеживания состояния Circuit Breaker

## 6. Сеть и балансировка нагрузки

### 6.1 Глобальная балансировка нагрузки
...
### 6.2 Сетевая изоляция и безопасность
...
### 6.3 Глобальная сеть и соединения
...
## 7. Хранение данных и резервное копирование

### 7.1 Многоуровневое хранилище данных
...
### 7.2 Стратегии репликации данных
...
**Мониторинг компонентов обработки ошибок:**
- **Circuit Breaker:** Отслеживание переходов между состояниями, количество срабатываний
- **Dead Letter Queue:** Количество сообщений в DLQ, время нахождения сообщений
- **Идемпотентность:** Отслеживание дублирующих операций, использование Idempotency-Key
- **TTL-резервирования:** Мониторинг автоматического освобождения ресурсов
- **Компенсирующие транзакции:** Успешность выполнения компенсирующих действий

### 8.2 Distributed tracing и logging
...
## 11. Cost management и оптимизация

### 11.1 Модель расходов
...
## 12. Disaster Recovery и Business Continuity
...
### 12.2 Тестирование Disaster Recovery
...
## 13. Резюме инфраструктурной архитектуры
