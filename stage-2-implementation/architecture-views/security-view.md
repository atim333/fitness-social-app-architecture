...
## 11.5 Безопасность Event Sourcing и Actor Model

### 11.5.1 Защита Event Streams

**Шифрование доменных событий:**

**Уровни шифрования событий:**
- **Заголовки событий:** Всегда открытые (eventId, timestamp, aggregateId, version)
- **Метаданные событий:** Шифруются с использованием контекстных ключей (correlationId, causationId)
- **Полезная нагрузка (payload):** 
  - Уровень 1 (High sensitivity): Полное шифрование для PII и финансовых данных
  - Уровень 2 (Medium sensitivity): Селективное шифрование чувствительных полей
  - Уровень 3 (Low sensitivity): Открытый доступ для публичных данных

**Key management для событий:**
- **Per-aggregate ключи:** Уникальные ключи шифрования для каждого агрегата
- **Key derivation:** Мастер-ключи + aggregateId = производный ключ
- **Ротация ключей:** Автоматическая ротация при изменении версии схемы события
- **Key archival:** Хранение старых ключей для дешифрования исторических событий

**Контроль доступа к потокам событий:**

**Авторизация для чтения событий:**
- **Read models:** Только сервисы с соответствующими правами могут подписываться на топики
- **Аудит:** Специальные сервисы аудита с правами только на чтение
- **Аналитика:** Сервисы аналитики с доступом только к деидентифицированным данным
- **Администраторы:** Полный доступ для отладки и мониторинга

**Транспортная безопасность:**
- **TLS 1.3:** Для всех соединений с Kafka брокерами
- **Client authentication:** Mutual TLS для сервисов-продюсеров и консьюмеров
- **SASL/SCRAM:** Для дополнительного уровня аутентификации
- **IP whitelisting:** Разрешение подключений только из доверенных подсетей

### 11.5.2 Аудит через Immutable Event Log

**Полнота аудита через события:**
- **Immutable log:** Все изменения состояния фиксируются в виде событий
- **Воспроизводимость:** Возможность восстановить состояние системы на любой момент времени
- **Non-repudiation:** Цифровая подпись событий обеспечивает неотказуемость
- **Chain of custody:** Полная цепочка изменений для расследований

**Структурированные события аудита:**

**Поля события аудита:**
- eventId: Уникальный идентификатор события аудита
- eventType: Тип события (SecurityAudit, BusinessAudit, SystemAudit)
- timestamp: Временная метка события
- aggregateType: Тип агрегата (Order, Payment, User)
- aggregateId: Идентификатор агрегата
- version: Версия агрегата
- actor: Информация об инициаторе (userId, userRole, ipAddress, userAgent, sessionId)
- action: Выполненное действие
- previousState: Предыдущее состояние агрегата
- newState: Новое состояние агрегата
- reason: Причина изменения
- signature: Цифровая подпись для верификации

**Категории аудируемых событий:**
- **Бизнес-события:** Все доменные события с метаданными безопасности
- **События доступа:** Логины, логауты, попытки несанкционированного доступа
- **События изменений:** Изменения конфигураций, прав доступа, политик
- **Системные события:** Ошибки, предупреждения, сбои

### 11.5.3 Безопасность Actor Model

**Изоляция состояния акторов:**

**Границы безопасности акторов:**
- **ActorRef как capability:** Доступ к актору только через ActorRef
- **Message-based security:** Проверка прав отправителя для каждого сообщения
- **Supervision boundaries:** Иерархии супервизии соответствуют доменным границам
- **Location transparency:** Безопасность не зависит от физического расположения актора

**Защита сообщений между акторами:**

**Шифрование сообщений:**
- **Внутри кластера:** TLS для всех сообщений между узлами Akka Cluster
- **Сериализация с шифрованием:** Зашифрованная сериализация для чувствительных данных
- **Digital signatures:** Подпись сообщений для проверки целостности и источника
- **Replay protection:** Уникальные идентификаторы сообщений и временные метки

**Контроль доступа к акторам:**

**Ролевой доступ для акторов:**
- **System actors:** Только системные компоненты могут взаимодействовать
- **User actors:** Проверка прав пользователя перед обработкой сообщений
- **Service actors:** Аутентификация сервисов через сертификаты
- **Administrative actors:** Строгая аутентификация и авторизация

### 11.5.4 Безопасность CQRS Read Models

**Защита Read Models:**

**Изоляция данных чтения:**
- **Data segregation:** Раздельные БД для разных уровней чувствительности
- **Row-level security:** Фильтрация данных на уровне запросов
- **Column-level encryption:** Шифрование отдельных колонок с чувствительными данными
- **Dynamic data masking:** Маскирование данных в зависимости от прав пользователя

**Контроль доступа к Read Models:**

**Авторизация запросов:**
- **Query validation:** Проверка прав пользователя перед выполнением запроса
- **Result filtering:** Фильтрация результатов на основе ролей и атрибутов
- **Rate limiting:** Ограничение частоты запросов для предотвращения злоупотреблений
- **Audit logging:** Логирование всех запросов к Read Models

**Кэширование и безопасность:**

**Безопасное кэширование:**
- **Encrypted cache:** Шифрование кэшированных данных
- **Cache isolation:** Раздельные кэши для разных категорий пользователей
- **Cache invalidation:** Принудительное обновление кэша при изменении прав доступа
- **TTL policies:** Время жизни кэша на основе чувствительности данных

### 11.5.5 Соответствие регуляторным требованиям

**GDPR compliance для Event Sourcing:**
- **Right to be forgotten:** Возможность удаления всех событий пользователя
- **Data portability:** Экспорт всех событий в структурированном формате
- **Purpose limitation:** События хранятся только для явно определенных целей
- **Storage limitation:** Автоматическое удаление событий по истечении срока хранения

**PCI DSS для финансовых событий:**
- **Card data isolation:** Отдельные топики для событий с платежными данными
- **Encryption requirements:** Соответствие стандартам шифрования PCI DSS
- **Access logging:** Полный аудит доступа к финансовым событиям
- **Regular testing:** Тестирование безопасности финансовых потоков событий

**HIPAA для медицинских данных:**
- **PHI protection:** Специальная обработка защищенной медицинской информации
- **Audit controls:** Требования к аудиту доступа к медицинским данным
- **Transmission security:** Шифрование при передаче медицинских событий
- **Business associate agreements:** Контрактные обязательства с партнерами
...
=======
требованиям, а также устойчивость к современным и emerging угрозам.
