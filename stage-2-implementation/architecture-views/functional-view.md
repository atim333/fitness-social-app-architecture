...
### 6.1 Управление транзакциями и идемпотентностью (Агрегат `Transaction`)
...
### 6.2 Управление TTL и временными данными (Агрегат `TTLRecord`)
...
### 6.3 Массовая обработка и очереди (Агрегат `BatchJob`)
