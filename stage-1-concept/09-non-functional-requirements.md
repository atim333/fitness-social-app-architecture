...
### 3.5 Защита данных в хранилищах

**S5.1:** Все чувствительные данные пользователей (данные о здоровье, платежная информация, контактные данные) должны шифроваться при хранении в базах данных.
**S5.2:** Доступ к базам данных должен быть ограничен только сервисами backend-подсети.
**S5.3:** Защита от SQL-инъекций через использование параметризованных запросов и ORM.
**S5.4:** Шифрование связи между всеми внутренними сервисами (mTLS).
**S5.5:** Регулярный аудит доступа к чувствительным данным с хранением логов 90 дней.
...
## 10. Требования к инфраструктуре и отказоустойчивости

### 10.1 Архитектура развертывания
- **INF.1:** Использование Kubernetes для оркестрации контейнеров с минимум 3 нодами в каждом кластере
- **INF.2:** Геораспределение по минимум 2 регионам для обеспечения отказоустойчивости
- **INF.3:** Global Load Balancer (GSLB) для автоматического переключения трафика между регионами
- **INF.4:** Репликация базы данных между регионами (синхронная в основном регионе, асинхронная в дополнительном)

### 10.2 Доступность и восстановление
- **INF.5:** Общая доступность системы (SLA): 99.9% (≤8.76 часов простоя в год)
- **INF.6:** Recovery Time Objective (RTO): ≤30 минут для критичных сервисов
- **INF.7:** Recovery Point Objective (RPO): ≤5 минут для потери данных
- **INF.8:** Автоматическое восстановление (self-healing) контейнеров при сбоях

### 10.3 Масштабируемость инфраструктуры
- **INF.9:** Автоматическое горизонтальное масштабирование при CPU >70%
- **INF.10:** Увеличение вычислительных мощностей на 200% в течение 10 минут
- **INF.11:** Поддержка до 500,000 одновременных активных пользователей в одном регионе
- **INF.12:** Multi-AZ развертывание в каждом регионе (минимум 3 Availability Zones)

### 10.4 Стратегии развертывания
- **INF.13:** Canary-развертывание для новых функций (5% → 25% → 50% → 100% трафика)
- **INF.14:** Blue-Green развертывание для мажорных обновлений
- **INF.15:** Rolling updates для минорных исправлений (maxUnavailable=1, maxSurge=25%)
- **INF.16:** Feature flags для управления включением функционала без деплоя

### 10.5 Тестовые окружения
- **INF.17:** Поддержка 5 уровней тестовых окружений:
  1. **Development** - локальные окружения разработчиков
  2. **Integration** - автоматические тесты и проверка взаимодействия
  3. **Staging** - полная копия production (меньший масштаб)
  4. **Pre-production/Canary** - тестирование на реальных данных
  5. **Disaster Recovery** - стенд для проверки восстановления
- **INF.18:** Использование одного Docker-образа во всех окружениях (build once, deploy everywhere)

### 10.6 Disaster Recovery Plan
- **INF.19:** Приоритеты восстановления:
  1. Критические сервисы (5 минут): Identity, Workout, Payment
  2. Полукритические сервисы (15 минут): Social, Gamification
  3. Некритические сервисы (60 минут): Analytics, Recommendations
- **INF.20:** Регулярные учения по восстановлению (квартально)
- **INF.21:** Автоматическое тестирование бэкапов (еженедельно)

### 10.7 Мониторинг и алертинг
- **INF.22:** Полный стек мониторинга (Prometheus, Grafana, Alertmanager)
- **INF.23:** Мониторинг SLO/SLA с автоматическим алертированием
- **INF.24:** Централизованное логирование с хранением 90 дней горячих и 1 года холодных логов
- **INF.25:** Мониторинг состояния Kubernetes кластера и автоматическое исправление проблем

## 11. Требования к наблюдаемости

### 11.1 Бизнес-ориентированные метрики
- **OBS.1:** Система должна собирать ключевые бизнес-показатели в реальном времени:
  - Активные тренировки в минуту (ATM)
  - Коэффициент вовлеченности пользователей (DAU/MAU)
  - Конверсия в покупки товаров
  - Эффективность промоакций по регионам
- **OBS.2:** Метрики должны быть доступны на дашбордах для бизнес-пользователей (маркетинг, продажи) с обновлением не реже чем раз в 5 минут.

### 11.2 Многоуровневое логирование
- **OBS.3:** Централизованный сбор логов со всех микросервисов в структурированном формате (JSON).
- **OBS.4:** Разделение каналов данных для различных типов событий:
  - Телеметрия тренировок (задержка <100мс)
  - События безопасности (гарантированная доставка)
  - Бизнес-события (покупки, регистрации) для аналитики
- **OBS.5:** Логи должны содержать идентификаторы корреляции (correlation ID) для отслеживания запросов через несколько сервисов.

### 11.3 Стратегия хранения логов
- **OBS.6:** Многоуровневое хранение логов:
  - Горячее хранилище (7 дней): Elasticsearch для оперативного поиска и отладки
  - Теплое хранилище (30 дней): TimescaleDB для аналитики и отчетности
  - Холодное хранилище (1 год): S3-совместимое хранилище для аудита и compliance
- **OBS.7:** Специальные требования для определенных типов данных:
  - Данные о здоровье пользователей: хранение 5+ лет в соответствии с медицинскими стандартами
  - Финансовые транзакции: хранение 3 года для аудита
  - Логи безопасности: хранение в WORM-хранилище (Write Once Read Many) для обеспечения неизменяемости.

### 11.4 Трассировка распределенных транзакций
- **OBS.8:** Сквозное отслеживание запросов через все сервисы с использованием OpenTelemetry.
- **OBS.9:** Корреляция трассировок с бизнес-идентификаторами (user_id, workout_id, challenge_id).
- **OBS.10:** Замер времени выполнения критических операций (например, создание тренировки, обработка платежа).

### 11.5 Предотвращение антипаттернов наблюдаемости
- **OBS.11:** Избегать "логирования как мусорного бака" - логи должны быть структурированными и содержать только полезную информацию.
- **OBS.12:** Не допускать "алертного ада" - алерты должны быть приоритизированы и агрегированы, чтобы не более 10 критических алертов в день.
- **OBS.13:** Бизнес-метрики должны иметь приоритет над техническими (например, DAU важнее загрузки CPU).

## Приоритеты требований

| Категория | Приоритет | Обоснование |
|-----------|-----------|-------------|
| **Инфраструктура и отказоустойчивость** | **Высший** | **Глобальная доступность и быстрая реакция на сбои критичны для удержания пользователей во время массовых событий и промоакций. Геораспределение и автоматическое восстановление обеспечивают бесперебойную работу приложения.** |
| **Наблюдаемость** | **Высокий** | **Возможность отслеживать состояние системы и бизнес-показатели в реальном времени критична для оперативного реагирования на проблемы и принятия решений на основе данных.** |
...
10. **Тестирование отказоустойчивости:** Проведение Chaos Engineering экспериментов (отключение нод, регионов, сетевые задержки)
11. **Тестирование восстановления:** Регулярные учения по Disaster Recovery с измерением RTO/RPO
12. **Нагрузочное тестирование инфраструктуры:** Тестирование автоматического масштабирования и переключения между регионами
13. **Тестирование стратегий деплоя:** Проверка Canary и Blue-Green развертываний в staging окружении
14. **Тестирование наблюдаемости:** Проверка сбора бизнес-метрик, логов и трассировок, а также работы дашбордов.
