...
| Конфигурации и эксперименты | Eventual | Динамическое обновление feature flags | Redis кэш + асинхронная синхронизация |
...
### 9. Сервис конфигураций и экспериментов (Configuration Service)
- **Назначение:** Централизованное управление feature flags, A/B тестирование промоакций
- **Подход:** Гибридный (Configuration Service + локальное кэширование)
- **Применение:** A/B тесты для челленджей, управление региональными промоакциями
- **Интеграция:** С User Service для сегментации, с API Gateway для маршрутизации

### 10. Сервис безопасности и шифрования (Security Service)
- **Назначение:** Управление аутентификацией, авторизацией, шифрование чувствительных данных
- **Интеграция:** Со всеми сервисами через JWT токены, централизованное управление ключами шифрования
- **Функции:** RBAC, аудит доступа, защита от инъекций, управление сессиями

### 11. Система наблюдаемости и мониторинга (Observability Stack)
- **Назначение:** Централизованный сбор метрик, логов и трассировок для всех компонентов системы, обеспечение видимости работы приложения и бизнес-процессов.
- **Соответствующий DDD контекст:** Cross-cutting concern (сквозной аспект, пересекающий все контексты)
- **Компоненты:**
  - **Сбор данных:** OpenTelemetry агенты во всех микросервисах, FluentBit/Fluentd для логов
  - **Обработка:** Apache Kafka для буферизации и маршрутизации потоков данных с разделением на каналы:
    - Телеметрия (задержка <100мс для данных тренировок)
    - Логи приложений (гарантированная доставка)
    - Бизнес-события (event-driven для аналитики)
  - **Хранение:**
    - Prometheus + Thanos для метрик (30 дней горячих, 1 год холодных)
    - Elasticsearch для логов (7 дней горячих + 30 дней теплых + 1 год холодных)
    - Jaeger для распределенных трассировок
    - S3-совместимое хранилище для архивных данных (5+ лет для медицинских данных)
  - **Визуализация и алертинг:**
    - Grafana с дашбордами для технических и бизнес-метрик
    - AlertManager для управления алертами с приоритезацией
    - Kibana для анализа логов
- **Ключевые метрики:**
  - **Технические:** Latency (p95, p99), error rate, throughput, CPU/memory usage
  - **Бизнес-метрики:**
    - Активные пользователи в минуту (ATM), DAU/MAU
    - Завершенные тренировки, средняя продолжительность
    - Конверсия в покупки, эффективность промоакций
    - Участие в челленджах, прогресс достижений
  - **Метрики здоровья системы:** SLA, RTO, RPO, время восстановления
- **Трассировка:** Сквозное отслеживание запросов с корреляцией по business_id (user_id, workout_id, challenge_id)
- **Принципы:**
  - **Бизнес-ориентированность:** Приоритет бизнес-метрик над техническими
  - **Структурированное логирование:** JSON формат с обязательными полями (timestamp, level, service, trace_id, user_id, business_id)
  - **Прогрессивная детализация:** От дашбордов реального времени до детальных отчетов для расследования инцидентов
  - **Предотвращение антипаттернов:** Избегание "алертного ада", "логирования как мусорного бака"
- **Интеграция:** Со всеми сервисами через стандартные протоколы (OTLP, Prometheus metrics, structured logging)
...
- **Топики по доменам:**
  ...
  - `observability-events` - события для системы наблюдаемости (метрики, логи, трассировки)
...
- `BusinessMetricRecordedEvent` → запись ключевых бизнес-показателей
- `SystemHealthEvent` → мониторинг состояния системы
- `UserBehaviorTrackedEvent` → отслеживание поведения пользователей для аналитики
...
### Стратегии развертывания и отказоустойчивости

#### Требования к доступности
- **SLA:** 99.9% доступности (менее 8.76 часов простоя в год)
- **RTO (Recovery Time Objective):** ≤ 30 минут для критичных сервисов
- **RPO (Recovery Point Objective):** ≤ 5 минут для потери данных

#### Геораспределение и отказоустойчивость
- Минимум 2 региона с активными Kubernetes кластерами
- Global Load Balancer для автоматического переключения трафика при отказе региона
- База данных с синхронной репликацией в основном регионе и асинхронной в дополнительном

#### Архитектура Kubernetes кластера (на регион)
- **Control Plane:** 3 master nodes (для обеспечения кворума)
- **Worker Nodes:** 10-20 worker nodes с автоскейлингом (CPU >70% → +2 nodes)
- **Ingress:** 3 ноды Ingress Controller (Nginx/Traefik)
- **Service Mesh:** Istio/Linkerd для трассировки и балансировки

#### Тестовые окружения и CI/CD
1. **Development:** Локальные окружения разработчиков (Minikube/Docker Desktop)
2. **Integration:** Автоматическое тестирование (API, contract, интеграционные тесты)
3. **Staging:** Полная копия production (меньший масштаб, production-like данные)
4. **Production:** Рабочий кластер с multi-AZ, multi-region развертыванием

#### Стратегии деплоя
- **Canary-развертывание:** Для новых функций (5% трафика → мониторинг 24ч → 25% → 50% → 100%)
- **Blue-Green:** Для мажорных обновлений без обратной совместимости
- **Rolling updates:** Для минорных исправлений (maxUnavailable=1, maxSurge=25%)

#### Приоритеты восстановления (Disaster Recovery Plan)
1. **Уровень 1 (0-5 мин):** Переключение GSLB на другой регион, восстановление критичных сервисов
2. **Уровень 2 (5-30 мин):** Восстановление полукритических сервисов, валидация данных
3. **Уровень 3 (30-60 мин):** Полное восстановление всех сервисов, пост-мортем анализ
