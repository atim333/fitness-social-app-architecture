...
### Инфраструктура и наблюдаемость для MVP:
- **Контейнеризация:** Docker для всех сервисов
- **Оркестрация:** Минимальный Kubernetes кластер (3 ноды) для отказоустойчивости
- **Мониторинг:** Базовый стек (Prometheus для метрик, Grafana для визуализации)
- **Логирование:** Централизованный сбор логов со всех сервисов
- **Метрики:** Технические (запросы/сек, время ответа) + базовые бизнес-метрики (регистрации, активные пользователи)
...
- **Инфраструктурные метрики:** Доступность API ≥ 99.5%, время восстановления ≤ 15 минут
...
### 6. Система инфраструктуры и наблюдаемости
- **Критичность:** Высокая
- **Обоснование:** Обеспечивает отказоустойчивость, масштабируемость и видимость работы системы
- **Компоненты:** Kubernetes, Prometheus/Grafana, централизованное логирование
- **План масштабирования:** Геораспределение, автоматическое восстановление
