...
**Риск #T-017: Недостаточная отказоустойчивость инфраструктуры**

- **Описание:** Невозможность обеспечения SLA 99.9% из-за недостаточной геораспределенности или неправильных стратегий развертывания
- **Вероятность:** Средняя (3/5)
- **Воздействие:** Критическое (5/5) - прямой ущерб репутации и потеря пользователей
- **Рисковый балл:** 15/25
- **Стратегия митигации:**
  - Внедрение Kubernetes кластеров в минимум 2 географических регионах
  - Настройка Global Load Balancer с автоматическим failover
  - Реализация Disaster Recovery Plan с четкими RTO (≤30 мин) и RPO (≤5 мин)
  - Регулярные учения по восстановлению (квартально)
  - Использование Canary и Blue-Green стратегий развертывания
- **Компромисс:** Увеличение стоимости инфраструктуры на 40-60%, сложность управления распределенными кластерами
...
**Риск #O-007: Недостаточность бизнес-ориентированной наблюдаемости**

- **Описание:** Система мониторинга фокусируется на технических метриках, игнорируя бизнес-показатели, что затрудняет принятие решений
- **Вероятность:** Высокая (4/5) - типичная проблема при внедрении мониторинга
- **Воздействие:** Высокое (4/5) - невозможность оценки эффективности функций и промоакций
- **Рисковый балл:** 16/25
- **Стратегия митигации:**
  - Внедрение бизнес-метрик в реальном времени (активные тренировки/мин, DAU/MAU, конверсии)
  - Создание дашбордов для бизнес-пользователей (маркетинг, продажи)
  - Трассировка распределенных транзакций с корреляцией по business_id
  - Многоуровневое хранение логов (горячие/теплые/холодные) для разных целей
  - Предотвращение "алертного ада" через приоритезацию и агрегацию
- **Компромисс:** Увеличение объема данных и стоимости хранения, необходимость обучения бизнес-пользователей
...
**Риск #R-004: Нарушение compliance при хранении логов и метрик**

- **Описание:** Логирование и сбор метрик могут нарушать требования GDPR/HIPAA при неправильном хранении чувствительных данных
- **Вероятность:** Средняя (3/5)
- **Воздействие:** Высокое (4/5) - штрафы и репутационные потери
- **Рисковый балл:** 12/25
- **Стратегия митигации:**
  - Разделение каналов данных: телеметрия, логи, метрики с разными политиками хранения
  - Многоуровневое хранение: горячие (7 дней), теплые (30 дней), холодные (1+ год)
  - Автоматическая маскировка PII данных в логах
  - WORM-хранилище (Write Once Read Many) для аудитных логов
  - Регулярный аудит доступа к логам и метрикам
- **Компромисс:** Усложнение системы логирования, увеличение задержки при поиске в логах

### 6.2 Риски лицензирования и intellectual property

**Риск #R-005: Лицензирование third-party компонентов**
...
7. **Риск #T-017:** Недостаточная отказоустойчивость инфраструктуры (15/25)
8. **Риск #O-007:** Недостаточность бизнес-ориентированной наблюдаемости (16/25)
...
- **Балансировать:** Стоимость инфраструктуры высокой доступности vs требования SLA 99.9%
- **Инвестировать:** В бизнес-ориентированную наблюдаемость для data-driven решений
...
5. Разработать и утвердить Disaster Recovery Plan с RTO/RPO метриками
6. Настроить сбор ключевых бизнес-метрик (активные пользователи, конверсии)
...
5. Реализовать геораспределение Kubernetes кластеров в 2+ регионах
6. Внедрить многоуровневое хранение логов с compliance требованиями
